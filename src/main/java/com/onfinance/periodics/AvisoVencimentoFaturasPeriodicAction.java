 ...
 */
public class AvisoVencimentoFaturasPeriodicAction {

    private final String emailFrom;

    public AvisoVencimentoFaturasPeriodicAction(PeriodicActionEntity periodic) {
        emailFrom = PropertyUtil.get("email.service.email.from");
    }

    public void execute() throws Exception {

        LogUtil.getLogger().log(Level.INFO, "{0}: {1}", new Object[]{LocalDateTime.now(), "Executando AvisoVencimentoFaturasPeriodicAction...."});

        try ( HibernateSession session = HibernateSession.getHibernateSession()) {
            PeriodicActionEntity periodic = session.get(PeriodicActionRepository.class).findByNome("AvisoVencimentoFaturasPeriodicAction");
            ...
            if (periodic.getUltimaExecucao().toLocalDate().isBefore(LocalDate.now())) {
                ...
                List<PagamentoEntity> faturas = session.get(PagamentoRepository.class).findPagamentosByFiltro(inicio, fim, "A", "S", List.of("TD"), 0, 0);
                ...
                faturas.forEach((fatura) -> {
                    try {
                        ...
                        if (perfil.isAvisoFaturaAVencer()) {
                            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
                            String vencimento = fatura.getFatura().getVencimento().format(formatter);
                            String subject = "Lembrete de Vencimento ".concat(fatura.getFatura().getEmpresa().getNome()).concat(" - Vencto ").concat(vencimento);
                ...
                LogUtil.getLogger().log(Level.INFO, "{0}: {1}", new Object[]{LocalDateTime.now(), "Finalizado a execução da AvisoVencimentoFaturasPeriodicAction...."});
            } else {
                LogUtil.getLogger().log(Level.INFO, "{0}: {1}", new Object[]{LocalDateTime.now(), "AvisoVencimentoFaturasPeriodicAction fora do período de execução...."});
            }
        }
    }
    ...
    private String message(PagamentoEntity fatura) {
        ...
        return "Prezado(a) CLIENTE,"
                .concat("\n \n")
                .concat("Informamos que a fatura " + fatura.getFatura().getEmpresa().getNome() + " vence em " + vencimento + ".")
                .concat("\n \n")
