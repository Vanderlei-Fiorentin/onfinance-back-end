...
RETURNS TRIGGER AS $$
    ...
    DECLARE vProximoValor INTEGER;
BEGIN

    vVencimento := NEW.VENCIMENTO;
    ...
    SELECT L.TIPO_PAGTO, L.DT_LANCTO, L.ID_EMPRESA, L.ID_CARTAO, L.ID_CONTA
     ...
     WHERE L.ID_LANCTO = NEW.ID_LANCTO;
    ...
    IF(vCartao IS NULL AND vConta IS NOT NULL) THEN
        ...
        IF(vTipoPagamento IN('A','T')) THEN      
            -- Insere a fatura      
            INSERT INTO FATURAS VALUES(vFatura, vEmpresa, vVencimento, NEW.VALOR);
            -- Atualiza a parcela com os dados da fatura
            UPDATE PARCELAS_LANCTO_CONTABIL SET ID_FATURA = vFatura WHERE ID_PARCELA = NEW.ID_PARCELA;
            -- Insere o pagamento para gerar extrato
            SELECT COALESCE(MAX(ID_PAGAMENTO), 0) + 1
              INTO vPagamento
              FROM PAGAMENTOS;
            INSERT INTO PAGAMENTOS VALUES(vPagamento, vFatura, NULL, vConta, NEW.VALOR, 0, 0, 0, vVencimento, 'T');
            -- Atualiza a sequence da tabela
            SELECT setval('pagamentos_id_pagamento_seq', vPagamento) INTO vProximoValor;
        ELSE
            INSERT INTO FATURAS VALUES(vFatura, vEmpresa, vVencimento, NEW.VALOR);
            -- Atualiza a sequence da tabela
            SELECT setval('faturas_id_fatura_seq', vFatura) INTO vProximoValor;
            -- Atualiza a parcela com os dados da fatura
            UPDATE PARCELAS_LANCTO_CONTABIL SET ID_FATURA = vFatura WHERE ID_PARCELA = NEW.ID_PARCELA;
        END IF;
    ...
    ELSE
        ...
        IF(vFatura > 0) THEN	
            ...
            UPDATE FATURAS 
               SET VALOR = (vValorFatura + NEW.VALOR) 
             WHERE ID_FATURA = vFatura;            
        ...
        UPDATE CARTOES_CREDITO 
           SET LIMITE_UTILIZADO = (vLimiteUtilizado + NEW.VALOR) 
...
CREATE OR REPLACE TRIGGER TRG_GERAR_FATURAS AFTER INSERT ON PARCELAS_LANCTO_CONTABIL 
FOR EACH ROW
EXECUTE PROCEDURE GERAR_FATURAS();